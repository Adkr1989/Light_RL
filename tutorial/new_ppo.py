...
def eval_loop(agent, env, render=False):
    s, info = env.reset()
    done = False
    total_rew = 0
    while not done:
...
def train(args):
    ...
    eval_env = gym.make(args.env_name, render_mode = "human" if args.render else None)
    ...
    # agent = PPO(actor, critic, buffer_size=args.buffer_size, learning_rate=args.learning_rate, num_episodes=args.num_episodes)
    agent = PPO(actor, critic, **vars(args.ppo_args))

    save_dir = osp.join(args.save_dir, f'ppo_{args.env_name}_new')
    ...
    while total_steps < args.max_timesteps:
        ...
        for _ in range(max_len):
            ...
            if total_steps % args.eval_interval == 0:
                eval_rews = eval_policy(agent, eval_env, loop_cnt=3)
                print(f'env: {args.env_name}, step: {int(total_steps / 1000)}k, rewards:{round(eval_rews, 3)}')

                live_time.append(eval_rews)
                Tools.plot(live_time, save_dir, title=f'PPO_{args.env_name}', x_label="Steps", y_label="Eval rewards", step_interval=10)
...
def parse_args():
    ...
    env_group.add_argument('--env_name', type=str, default='CartPole-v1', help='Name of the environment')
    ...
    env_group.add_argument('--train', action='store_true', help='Train')
    env_group.add_argument('--eval', action='store_true', help='Eval')
    ...
    train_group.add_argument('--learning_rate', type=float, default=1e-4, help='Learning rate for the optimizer')
    ...
    ppo_params = ['clip_ratio', 'gamma', 'lamda', 'entropy_coef', 'value_coef', 
